...
- **Auth/RBAC is decorator-driven, and the guards are global.** `AuthGuard`, `PermissionGuard` and `RoleGuard` are `APP_GUARD` providers registered in that order, so **every route is authenticated by default** and controllers carry no `@UseGuards`. Gate each method with `@PermissionAuth("user:create")` (conjunctive — all listed permissions required) or `@RoleAuth("superuser")` (disjunctive — any one). `@Public()` is the only opt-out, used by `AppController`, `HealthController`, and the six public auth routes. `@CurrentUser()` injects the resolved `UserInformation` (roles + flattened permissions), cached in Redis under `user:<id>` — and that entry is dropped by every write that changes identity or authorization, including the role paths, which invalidate every holder of the role.
